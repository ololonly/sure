...
# Shared currency conversion helpers for Bybit processors.
# Converts USD amounts to the family's configured base currency using
# ExchangeRate.find_or_fetch_rate (which has a built-in nearest-rate lookback).
# When a fallback or no rate is used, sets a stale flag in account.extra["bybit"].
module BybitAccount::UsdConverter
  private
