...
class BybitAccount::UsdConverterTest < ActiveSupport::TestCase
  ...
  test "build_stale_extra returns correct hash when stale" do
    ...
    assert_equal({ "bybit" => { "stale_rate" => true, "rate_date_used" => "2026-03-25", "rate_target_date" => "2026-03-28" } }, result)
  end
  ...
  test "build_stale_extra returns cleared hash when not stale" do
    ...
    assert_equal({ "bybit" => { "stale_rate" => false } }, result)
  end
end
